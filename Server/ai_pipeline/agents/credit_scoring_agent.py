...
- If NO rating exists: score 20 (Severe uncertainty penalty - DEMAND MORE DATA)""",
    },
    "promoter_background": {
...
- 0-29: Wilful defaulter, fraud cases, arrested directors""",
    ...
    "regulatory_compliance": {
        ...
        "prompt": """Evaluate REGULATORY COMPLIANCE with deep focus on Indian Corporate standards.
        
Score 0-100 based on:
- GST filing regularity (GSTR-1, GSTR-3B on time). Penalize any gap > 1 month.
- GSTR-1 vs 3B consistency (Any mismatch is a major red flag).
- MCA annual return filing status (AOC-4, MGT-7).
- Income tax disputes (check for 'Demand' or 'Notice' in financials).
- CIBIL Commercial record (look for 'Written Off' or 'Default' status).
- SEBI compliance (for listed companies).
- Environmental/labor law adherence.

Scoring guide:
- 90-100: Flawless compliance, no GSTR mismatches, clean CIBIL.
- 70-89: Minor delays (1-15 days), no critical notices.
- 50-69: Active disputes but provisions made in balance sheet.
- 30-49: Multiple GSTR mismatches or CIBIL search reveals past defaults.
- 0-29: Legal prosecution, MCA strike-off threat, or CIBIL 'Default' status.""",
    },
    "litigation_risk": {
...
- 0-29: Weak position in declining sector, no competitive edge""",
    ...
    "esg_irregularities": {
        ...
        "prompt": """Evaluate ESG RISKS & FINANCIAL IRREGULARITIES (Fraud Detection).
        
Score 0-100 based on (INVERT — higher score = FEWER problems):
- **Circular Trading Detection**: Look for sales inflated by group concerns or revenue not matching Bank Statement credits.
- **Data Mismatches**: GSTR-3B vs Bank Credits vs Balance Sheet Revenue. (Any >10% variance = Critical Flag).
- Environmental violations or pollution cases.
- Social issues (labor disputes, safety incidents).
- Governance concerns (Board independence, audit qualification).
- Related party transaction anomalies or high promoter loans.
- Contradiction between documents and web research (e.g., promoter claims vs court filings).

Scoring guide:
- 90-100: No irregularities, revenue verified across 3+ sources.
- 70-89: Minor ESG concerns (e.g., carbon footprint) but clean financials.
- 50-69: Minor data variances or audit notes with 'Emphasis of Matter'.
- 30-49: Indicators of revenue inflation or suspicious related-party flows.
- 0-29: Strong evidence of 'Circular Trading', fraud, or major fund siphoning.""",
    },
}
...
If data is missing for this category, score 0-20 (Severe penalty for lack of transparency).
DO NOT guess or be optimistic. If you don't see it, it doesn't exist."""
