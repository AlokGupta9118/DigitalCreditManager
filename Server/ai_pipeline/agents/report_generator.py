...
def _add_executive_summary(doc, company_name: str, research_text: str, scorecard_text: str, due_diligence_text: str = ""):
...
Based on the research findings, site visit observations, and scoring below, write a 4-5 paragraph executive summary covering:
1. Company introduction (1 sentence)
2. Key financial strengths
3. Key risks and concerns (include site visit findings if any)
...
def _add_due_diligence_section(doc, due_diligence_text: str):
    """Add a section for site visit findings and management credibility."""
    if not due_diligence_text or "No site visit" in due_diligence_text:
        return

    # Add a sub-header instead of a main header since it's nested in CHARACTER
    p = doc.add_paragraph()
    run = p.add_run("Site Visit Observations & Due Diligence")
    run.bold = True
    run.font.size = Pt(13)
    run.font.color.rgb = COLORS["primary"]

    # Capacity Utilization Info
    capacity_match = re.search(r'Capacity Utilization: (\d+)', due_diligence_text)
    if capacity_match:
        cap = capacity_match.group(1)
        p = doc.add_paragraph()
        run = p.add_run(f"Current Capacity Utilization: {cap}%")
        run.bold = True
        run.font.size = Pt(11)

    # Management Credibility
    cred_match = re.search(r'Management Credibility: (\w+)', due_diligence_text)
    if cred_match:
        cred = cred_match.group(1)
        p = doc.add_paragraph()
        run = p.add_run(f"Management Assessment: {cred}")
        run.bold = True
        run.font.color.rgb = COLORS["secondary"]

    # Body text
    for para in due_diligence_text.split("\n"):
        if any(keyword in para for keyword in ["Observations:", "Risks:"]):
            p = doc.add_paragraph()
            run = p.add_run(para.strip())
            run.font.size = Pt(11)
...
def _add_due_diligence_section(doc, due_diligence_text: str):
    """Add Due Diligence / Site Visit section."""
    _add_section_header(doc, "3. Due Diligence (Site Visit)")
    
    if not due_diligence_text or "No site visit" in due_diligence_text:
        p = doc.add_paragraph()
        run = p.add_run("No site visit data available for this assessment.")
        run.italic = True
        run.font.size = Pt(10)
        run.font.color.rgb = COLORS["muted"]
        return

    for line in due_diligence_text.strip().split("\n"):
        p = doc.add_paragraph()
        if ":" in line:
...
def _add_lending_recommendation(doc, scorecard_text: str, score: float, rating: str):
    ...
    # Add Loan Amount and Rate if requested
    for line in scorecard_text.split("\n"):
        if "Suggested Amount:" in line or "Suggested Loan Amount:" in line:
            amt = line.split(":")[-1].strip()
            p = doc.add_paragraph()
            run = p.add_run("Suggested Facility: ")
            run.bold = True
            run.font.size = Pt(12)
            run = p.add_run(amt)
            run.font.size = Pt(12)
            run.bold = True
        elif "Interest Rate:" in line or "Suggested Rate:" in line:
            rate = line.split(":")[-1].strip()
            p = doc.add_paragraph()
            run = p.add_run("Pricing (ROI): ")
            run.bold = True
            run.font.size = Pt(11)
            run = p.add_run(rate)
            run.font.size = Pt(11)
...
def generate_report(
    ...
    # ── Build the report (The FIVE Cs Framework) ──
    print(f"\n  🔨 Building Five Cs framework...")
    ...
    _add_executive_summary(doc, company_name, research_text, scorecard_text, due_diligence_text)

    # 3. THE FIVE Cs
    # I. CHARACTER
    section_num = 1
    _add_section_header(doc, f"{section_num}. CHARACTER", "Management integrity & credit history")
    _add_due_diligence_section(doc, due_diligence_text)
    if "PROMOTER BACKGROUND" in research_sections:
        _add_research_section(doc, "Promoter Track Record", section_num, research_sections["PROMOTER BACKGROUND"])
    if "LITIGATION RISK" in research_sections:
        _add_research_section(doc, "Litigation & Legal Profile", section_num, research_sections["LITIGATION RISK"])

    # II. CAPACITY
    section_num += 1
    _add_section_header(doc, f"{section_num}. CAPACITY", "Cash flows, revenue stability and GST trends")
    if "FINANCIAL ANALYSIS" in research_sections:
        _add_research_section(doc, "Financial Performance Analysis", section_num, research_sections["FINANCIAL ANALYSIS"])
    elif "FINANCIAL OBSERVATIONS" in research_sections:
        _add_research_section(doc, "Financial Performance Analysis", section_num, research_sections["FINANCIAL OBSERVATIONS"])
    if "REGULATORY RISK" in research_sections:
        _add_research_section(doc, "Operational & Regulatory Compliance", section_num, research_sections["REGULATORY RISK"])

    # III. CAPITAL
    section_num += 1
    _add_section_header(doc, f"{section_num}. CAPITAL", "Equity cushion, net worth and leverage ratios")
    # Scorecard table fits well here to show quantitative capital strength
    _add_scorecard_table(doc, scorecard_text)

    # IV. COLLATERAL
    section_num += 1
    _add_section_header(doc, f"{section_num}. COLLATERAL", "Security structure and primary/secondary covers")
    doc.add_paragraph("Documentation Pending: Final security structure to be determined upon facility sanctioning. Standard hypothecation of current assets and fixed assets recommended.")

    # V. CONDITIONS
    section_num += 1
    _add_section_header(doc, f"{section_num}. CONDITIONS", "Final recommendation and lending covenants")
